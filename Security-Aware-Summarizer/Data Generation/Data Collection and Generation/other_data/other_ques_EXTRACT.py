...
dataset=pd.read_csv("/home/shagoto/my_codes/other_data/all_ques(alpaca,adv,gptfuzz,jbb,xstest).csv")
...
strictly output format:
Summary: Summarization of prompt focusing on the keywords given, max 4 lines
Classification: Just the class name
Classification reason: The prompt is (class name) because (the clear reason of classification )"""
...
torch.cuda.set_device(1)  # This will set the default device to GPU 1
...
pipeline = transformers.pipeline(
    ...
    device_map={"": 1}  # Specify to use GPU 1
)
...
for batch_idx in range(num_batches):
    ...
    # Process batch
    outputs = pipeline(batch_messages, max_new_tokens=256, temperature=0.3)
...
def summary_extractor(data):
  data=data.lower()
  one=data.split("summary:")[1]
  summary=one.split("classification:")[0]
  two=one.split("classification:")[1]
  classification=two.split("classification reason:")[0]
  classification_reason=two.split("classification reason:")[1]
  summary=summary.replace('\n', '').replace('*', '').strip()
  classification=classification.replace('\n', '').replace('*', '').strip()
  classification=re.sub(r'[^\w\s]', '', classification)
  classification = classification.lower()

  classification_reason=classification_reason.replace('\n', '').replace('*', '').strip()
  if classification.lower() not in classification_reason.lower():
    if classification.lower() not in summary.lower():
      classification_reason= " The prompt is " + classification + " because " + classification_reason
  summary = summary+ classification_reason
...
dataset.to_csv("./other_ques_llama3.1_70B_final_last_Attempt.csv")